...
exports.addToCart = async (req,res,next) => {
        ...
        if(requestBody != null){
            ...
            const productId = requestBody.productId;
            const addedProductQuantity = requestBody.quantity;
            //check if the product if present in the backend database
            if(userId !== null && userId !== undefined && productId !== null && addedProductQuantity !== null
                && productId !== undefined && addedProductQuantity !== undefined) {
                    try{
                        ...
                        if(products.length > 0){
                            ...
                            if(product.quantity > 0){
                                ...
                                if(addedProductQuantity <= productQuantity){
                                    ...
                                    try{
                                        ...
                                        if(existingCart.length > 0){
                                            ...
                                            if(productExists != null){
                                                ...
                                                const newQuantity = oldQuantity + addedProductQuantity;
                                                //const updateQuantity = { quantity : newQuantity };
                                                productExists.quantity = newQuantity;
                                                const newPrice =  product.price *  newQuantity;
                                                console.log("New Price: "+ newPrice);
                                                productExists.price = product.price *  newQuantity;
                                                
                                                //update the productQuantity
                                                const updateProductQuantity = product.quantity - addedProductQuantity;
                                                ...
                                                try{
                                                    ...
                                                    //     const responseClass = new ResponseClass(true,updatedCart,"quantity updated in the cart Successfully");
                                                    //     res.status(200).json({
                                                    //         response : responseClass.getResponse()
                                                    //     });
                                                    // }else{
                                                    //     const responseClass = new ResponseClass(false.valueOf, null, "Failure to update the product quantity in the cart");
                                                    //     res.status(409).json({
                                                    //         response : responseClass.getResponse()
                                                    //     })
                                                    // }
                                                        const responseClass = new ResponseClass(true, updateCart, "Cart Updated Successfully");
                                                        res.status(200).json({
                                                            response : responseClass.getResponse()
                                                        })
                                                }catch(err){
                                                ...
                                                const updateProductQuantity = productQuantity - addedProductQuantity;
        
                                                //push into the cart product List 
                                                cartProducts.push(addNewProductIntoExistingCart);
                                                console.log(typeof cartProducts.length);

                                                const updatedCartProductList = cartProducts;


                                                console.log("Length of the product" + cartProducts.length);
                                                //update the cart
                                                const updateProductCart = { products : cartProducts };
                ...
                const responseClass = new ResponseClass(false, null, "Fields are Empty");
            }
        }else{
    ...
    if(requestBody != null){
        ...
        try{
            ...
            if(cart != null && cart.length > 0){
                ...
                const cartProductsList = cart[0]['products']
                let totalPrice = 0;

                for(cartProduct = 0; cartProduct < cartProductsList.length; cartProduct++){
                    totalPrice = totalPrice + cartProductsList[cartProduct]['price']; 
                }
                result['totalPrice'] = totalPrice;
                result['product'] = cartProductsList;

                const responseClass = new ResponseClass(true,result,"cart details fetched successfully");
                res.status(200).json({
                    response : responseClass.getResponse()
                });
            }else{
    ...
    if(requestBody != null){
        ...
        try{
            ...
            if(cart.deletedCount != null && cart.deletedCount > 0){
                const responseClass = new ResponseClass(true,cart,"cart details deleted successfully");
                res.status(200).json({
                    response : responseClass.getResponse()
                });
            }else{
